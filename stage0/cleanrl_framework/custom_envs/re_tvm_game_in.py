...
class Blue:
    ...
    def update_states(self,decision_step:int,action_list:list)->None:
        ...
        for i in range(shoot_num):
            ...
            self.info=self.info+f"蓝方炮塔发射子弹_{i}，初始方向 {theta_list[i]}，速度 {v_list[i]}，分配伤害 {softmax_scores[i]}；"
            self.bullet_list.append(Bullet(self.x,self.y,theta_list[i],v_list[i],softmax_scores[i]))
        
        print("now bullet num",len(self.bullet_list))
    ...
    def update_states(self,decision_step)->None:
        ...
        for monster in self.monster_list:
            if not monster.alive:
                if monster_num>0:
                    monster.set_states()
                    self.info=self.info+f"红方生成一个进攻单位，位置 ({monster.x},{monster.y})，血量 {monster.hp}，"
                    monster_num-=1
            else:
