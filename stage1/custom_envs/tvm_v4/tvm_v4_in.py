...
class Blue:
    ...
    def update_states(self,decision_step:int,action_list:list)->None:
        ...
        shoot_list=[action_list[i] for i in range(0,17,4)]
        hit_list=[action_list[i] for i in range(1,17,4)]
        
        shoot_num=np.argmax(shoot_list)
        hit_set=hit_list
        theta_list=[action_list[i]*np.pi for i in range(2,17,4)] # [-1,1] 映射到 [-pi,pi]
        v_list=[action_list[i]*0.75+1.25 for i in range(3,17,4)] #[-1,1] 映射到[0,2]
    ...
    def get_states(self)->list:
        all_states=[self.x,self.y,self.available_bullet,self.hp]
...
class Monster:
    ...
    def set_states(self)->None:
        ...
        self.y=10
        self.v=random.uniform(0.1, 1)    # 固定速度绝对值
