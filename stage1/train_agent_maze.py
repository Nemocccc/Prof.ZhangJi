...
class Agent(nn.Module):
    def __init__(self, envs):
        ...
        self.global_encoder = nn.Sequential(
            layer_init(nn.Conv2d(1, 16, 2, stride=3)),
            nn.ReLU(),
    ...
    def get_value(self, x):
        ...
        global_obs=x[:,:-4].reshape(batch_size,1,15,15)/3
        local_obs=x[:,-4:]
        image=self.global_encoder(global_obs)
        vector=self.local_encoder(local_obs)
