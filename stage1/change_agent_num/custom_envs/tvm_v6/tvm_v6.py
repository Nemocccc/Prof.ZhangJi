...
from custom_envs.tvm_v6.tvm_v6_in import *
...
class TVM(gym.Env):
    ...
    def __init__(self, render_mode=None) -> None:
        ...
        self.action_space=gym.spaces.Box(low=-1.0, high=1.0, shape=(3,13), dtype=np.float32)

        self.blue_list=[Blue(x=0,y=0),Blue(x=5,y=0),Blue(10,0)]
        self.red=Red()
    ...
    def reset(self,seed=42,options=None):
        ...
        init_pos=[[0,0],[5,0],[10,0]]
        for blue,pos in zip(self.blue_list,init_pos):
            blue.reset(pos[0],pos[1])
        ...
        for blue in self.blue_list:
            blue_states_list.append(blue.get_states())
        
        real_red_states=red_states
        real_blue_states=blue_states_list

        all_states={"red_states":real_red_states,"blue_states_list":real_blue_states}
    ...
    def step(self,action_list):
        ...
        terminated=False
        truncated=False
        for action,blue in zip(action_list,self.blue_list):
            blue.update_states(self.decision_step,action)

        self.red.update_states(self.decision_step)

        for blue in self.blue_list:
            rew_info,terminated,truncated=check_red_blue(self.red,blue,self.decision_step)

        red_states=self.red.get_states()
        blue_states_list=[]
        for blue in self.blue_list:
            # print(blue.get_states())
            blue_states_list.append(blue.get_states())
    ...
    def render(self):
        ...
        for blue in self.blue_list:
            ...
            self.screen.blit(blue_img, (int(blue.x*80-35.5), int(800-blue.y*80-72)))

            # 基地上方显示HP
            blue_hp_text = self.font.render(f"HP: {blue.hp:.2f}", True, (0, 0, 0))
            self.screen.blit(blue_hp_text, (int(blue.x*80-35.5), int(800-blue.y*80-72) - 30)) 
