...
def qlearning_dataset(env, dataset=None, terminate_on_end=False, **kwargs):
    ...
    for episode_data in dataset.iterate_episodes():
        ...
        for i in range(N-1):
            obs = episode_data.observations[i].astype(np.float32)
            new_obs = episode_data.observations[i+1].astype(np.float32)
            action = episode_data.actions[i].astype(np.float32)
            reward = episode_data.rewards[i].astype(np.float32)
            terminations = episode_data.terminations[i]
            truncations = episode_data.truncations[i]
            done_bool = bool(terminations or truncations)
...
def train(args=get_args()):
    ...
    minari_dataset = minari.load_dataset("pendulum/sac-v0")
    dataset = qlearning_dataset(env,dataset=minari_dataset)
